...
public class Solution {
    ...
    private void changeIntoNumberOfRepetitions() {
        ...
        for (int j = 0; j < matrixOfPartExpr.size(); j++) { // 1 done
            ...
            /*for (int i = 1; i < list.size(); i++){
                System.out.println("Число, с которым мы работаем " + list.get(i-1));
                System.out.println("Число повторений для " + list.get(i-1) + " = " + numberOfRepetitions);
                if (list.get(i-1).equals(list.get(i))) // если числа одинаковы, то увеличиваем, иначе вставляем
                    numberOfRepetitions++;
                else {
    ...
    private void findTheSameColumns(){
        ...
        for (int i = 0; i < matrixOfPartExpr.size()-1;i++)
